...
public class GameScreen implements Screen {
    ...
    private void drawMobHealth(Mob mob) {
        ...
        float barWidth = Math.max(28f, b.width);
        float x = mob.getPosition().x - barWidth * 0.5f;
        float y = mob.visualTopY() + 4f;
        shapes.setColor(0f, 0f, 0f, 0.60f);
        shapes.rect(x, y, barWidth, 4f);
        shapes.setColor(0.86f, 0.08f, 0.08f, 0.95f);
        shapes.rect(x, y, barWidth * Math.max(0f, ratio), 4f);
    }
